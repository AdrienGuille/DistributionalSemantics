...
class Corpus:
    def __init__(self, source_file_path, max_nb_features=5000, window_size=5):
        ...
        for line in csv_reader:
            ...
            for i in range(nb_words):
                ...
                if self.vocabulary_map.get(words[i]) is not None:
                    ...
                    context_0 = words[start:i]
                    context_1 = words[i + 1:end + 1]
                    context_0.extend(context_1)
                    for word in context_0:
                        if self.vocabulary_map.get(word) is not None:
                            column_index = self.vocabulary_map[word]
                            # update co-occurrence count
                            count = 0
                            if (row_index, column_index) in self.X:
                                count = self.X[row_index, column_index]
                            self.X[row_index, column_index] = count + 1
